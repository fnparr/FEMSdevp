...
# *********************
#  constructors:  EventSeries() : (), (<contractType>, <rf_list> )

setGeneric(name = "EventSeries",
           def = function(contract, riskFactors){
...
setMethod(f = "EventSeries", signature = c("ContractType","list"),
          definition = function(contract, riskFactors){
            ptf <- Portfolio()
            ...
            cshfl_rslt1 <- generateEvents(ptf)[[1]] #first in single contract ptf
            ...
            for(evfield in env$Event_Field_Names) {
